...
/*}}}*/
    ...
    if(GENERATE_DATA){/*{{{*/
        ...
        filename.Form("%s_%s_%s/Asym_%s_%s.dat",EXPERIMENT.Data(),TARGET.Data(),PARTICLE.Data(), Comment1.Data(), Comment2.Data());
        ofstream outfile(filename);	
        // format of the pseudo-data
        // bin z x Q2 pt unpolarised_cross-section polarised_cross-section error_polarised_cross-section
        for( int i = 0; i < number_bins1; i++){
        ...
        filename.Form("%s_%s_%s/ChiSQ_%s_%s.dat",EXPERIMENT.Data(),TARGET.Data(),PARTICLE.Data(), Comment1.Data(), Comment2.Data());
        ofstream outfile2(filename);	
        outfile2<<Form("%s  %s  %f  %f  %f  %f  %f  %f  %f  %f",
                Comment1.Data(), Comment2.Data(), ChiSQ, BNLYit, Nuit, Ndit, auit, adit, buit, bdit);
        outfile2<<Form("  %f %f  %f  %f  %f  %f",
                Nuit_t, Ndit_t, auit_t, adit_t, buit_t, bdit_t)<<endl;
        outfile2.close();

...
%\begin{table}[bth!]
%\begin{tabular}{cccc}
%Variable      & Min  & Max & Bin Size & Comments  \\
%  $Q^{2}$    & 1.0~$GeV^{2}$ & 8.0~$GeV^{2}$ & 1.0~$GeV^{2}$ & 6 bins, last bin from 6 to 8 $GeV^{2}$
%  $z$           &  0.3~$GeV^{2}$  & 0.7~$GeV^{2}$ & 0.05~$GeV^{2}$ & 8 bins\\
%  $p_{T}$    &  0.0~$GeV$ &  1.6~$GeV$&  0.2~$GeV$ & $\leq$ 8 bins, double bin-size when total events $<5\times 10^{6}$ \\
%  $x$           & 0.1 & 0.7 & & bin size varies to keep events in one bin$\leq 10^{6}$ \\
%\end{tabular}

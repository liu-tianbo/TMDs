...
int read_data_clas(char const * directory = "", int particle_flag=1){/*{{{*/
    ...
    filename.Form("%s/projections_4D_%s_clas12.out",directory,particle.Data());
    ifstream infile(filename);
...
int main(int argc, char **argv)
{
    ...
    // Forth: Read in Coment#1
    TString Comment1= argv[4];

    // Fifth: Read in Coment#2
    TString Comment2= argv[5];

    // Sixth: Read in how many segments 
    int segs= int(strtod(argv[6], NULL));

    // Seventh: Read in which segment to use 
    int seg_used= int(strtod(argv[7], NULL));
    ...
    if(VERBOSE){
       ...
       cout<<Form("=== 13 Parameters from Inputs(%s,%s) : ===", Comment1.Data(), Comment2.Data())<<endl;
    ...
    //divide the projected data into N segments and only use one of the segment for each time - Z. Ye 06/26/2016
    const int seg_bins = number_bins/segs;
    double zval_used[seg_bins], xval_used[seg_bins], Q2val_used[seg_bins], ptval_used[seg_bins];
    for(int i=0; i<seg_bins;i++){
       int k = i + seg_bins * (seg_used);
       zval_used[i] = zval[k];
       xval_used[i] = xval[k];
       Q2val_used[i] = Q2val[k];
       ptval_used[i] = ptval[k];
    }

    if(VERBOSE) cout << "&& Total number of " << PARTICLE << " data points on target " << TARGET << " is " << number_bins << endl;
    if(VERBOSE) cout << "&& Divided into " << segs << " segments and only use the " << seg_used <<"th segment" << endl;
/*}}}*/
    ...
    if(GENERATE_UNPOL){/*{{{*/
        ...
        if (TARGET.EqualTo("3he") && PARTICLE.EqualTo("pip") )
            UNPOLARISED_N_PIP ( number_bins,  zval_used, xval_used, Q2val_used, ptval_used);
        if (TARGET.EqualTo("3he") && PARTICLE.EqualTo("pim") )
            UNPOLARISED_N_PIM ( number_bins,  zval_used, xval_used, Q2val_used, ptval_used);
        if (TARGET.EqualTo("p") && PARTICLE.EqualTo("pip") )
            UNPOLARISED_P_PIP ( number_bins,  zval_used, xval_used, Q2val_used, ptval_used);
        if (TARGET.EqualTo("p") && PARTICLE.EqualTo("pim") )
            UNPOLARISED_P_PIM ( number_bins,  zval_used, xval_used, Q2val_used, ptval_used);

        //WRITE ALL UNPOLARISED CROSS SECTIONS IN A DATA FILE
        TString filename; 

        filename.Form("./unpolarised_%s_%s_%s.dat",EXPERIMENT.Data(),TARGET.Data(),PARTICLE.Data());
        ...
        for( int i = 0; i < number_bins; i++){	
            outfile << i << " " << zval_used[i] << " " << xval_used[i] << " " << Q2val_used[i] << " " << ptval_used[i] << " " << UNPOLARISED[i] << endl;	
        }    
        outfile.close(); 
    }/*}}}*/
    ...
    if(GENERATE_DATA){/*{{{*/
        ...
        for( int i = 0; i < number_bins1; i++){
            int nbin =0;
            infile >> nbin >> zval_used[i] >> xval_used[i] >> Q2val_used[i] >> ptval_used[i] >> UNPOLARISED[i];

            if (x_min_jlab >= xval_used[i]) x_min_jlab = xval_used[i]; //min val_usedue at JLAB
            if (x_max_jlab <= xval_used[i]) x_max_jlab = xval_used[i]; //max val_usedue at JLAB
        ...
        if ( (EXPERIMENT.EqualTo("solid") || EXPERIMENT.EqualTo("clas") ) && TARGET.EqualTo("3he") && PARTICLE.EqualTo("pip") )
            POLARISED_N_PIP ( number_bins1,  zval_used, xval_used, Q2val_used, ptval_used);
        if ( (EXPERIMENT.EqualTo("solid") || EXPERIMENT.EqualTo("clas") ) && TARGET.EqualTo("3he") && PARTICLE.EqualTo("pim") )
            POLARISED_N_PIM ( number_bins1,  zval_used, xval_used, Q2val_used, ptval_used);
        if ( (EXPERIMENT.EqualTo("solid") || EXPERIMENT.EqualTo("clas") ) && TARGET.EqualTo("p") && PARTICLE.EqualTo("pip") )
            POLARISED_P_PIP ( number_bins1,  zval_used, xval_used, Q2val_used, ptval_used);
        if ( (EXPERIMENT.EqualTo("solid") || EXPERIMENT.EqualTo("clas") ) && TARGET.EqualTo("p") && PARTICLE.EqualTo("pim") )
            POLARISED_P_PIM ( number_bins1,  zval_used, xval_used, Q2val_used, ptval_used);/*}}}*/
        ...
        filename.Form("%s_%s_%s/NewAsym_%s_%s_%d_%d.dat",EXPERIMENT.Data(),TARGET.Data(),PARTICLE.Data(), Comment1.Data(), Comment2.Data() ,segs, seg_used);
        ofstream outfile(filename);
        ...
        filename.Form("%s_%s_%s/ChiSQ_%s_%s_%d_%d.dat",EXPERIMENT.Data(),TARGET.Data(),PARTICLE.Data(), Comment1.Data(), Comment2.Data() ,segs, seg_used);
        ofstream outfile2(filename);	
        outfile2<<Form("%s  %s  %12.4e  %12.4e  %12.4e  %12.4e  %12.4e  %12.4e  %12.4e",
                Comment1.Data(), Comment2.Data(), BNLYit, Nuit, Ndit, auit, adit, buit, bdit);

...
int main(){
	...
	ofstream tensor(Form("%s_tensor_charge_%s_%s.dat",Experiment.Data(),Target.Data(),Particle.Data()));
	ofstream trans(Form("%s_transversity_%s_%s.dat",Experiment.Data(),Target.Data(),Particle.Data()));

	cerr<<"--- Reading Data ..."<<endl;
	TString FileName = "";
	for(int i=0;i<N;i++){
		FileName = Form("../get_weight/results/more_Weight_%s_%s_%s_F%d.root",Experiment.Data(),Target.Data(),Particle.Data(),Scale[i]);
		...
		for(int j=0;j<N_W;j++){
			W->GetEntry(j);
			if(iW>-1.){
				deltaU_Exp[i] += 1./iCount * iU;	
				deltaD_Exp[i] += 1./iCount * iD;	
				deltaU_Exp_New[i] += iW * iU;	
				deltaD_Exp_New[i] += iW * iD;	

				deltaU_Exp_EX[i] += 1./iCount * iU_EX;	
				deltaD_Exp_EX[i] += 1./iCount * iD_EX;	
		...
		for(int j=0;j<N_W;j++){
			W->GetEntry(j);
			if(iW>-1.){
				deltaU_Vol[i] += 1./iCount* pow((iU-deltaU_Exp[i]),2);
				deltaD_Vol[i] += 1./iCount* pow((iD-deltaD_Exp[i]),2);
				deltaU_Vol_New[i] += iW * pow((iU-deltaU_Exp_New[i]),2);
				deltaD_Vol_New[i] += iW * pow((iD-deltaD_Exp_New[i]),2);

				deltaU_Vol_EX[i] += 1./iCount * pow((iU_EX-deltaU_Exp_EX[i]),2);
				deltaD_Vol_EX[i] += 1./iCount * pow((iD_EX-deltaD_Exp_EX[i]),2);
		...
		for(int l=0;l<100;l++){
			for(int j=0;j<N_W;j++){
				if(Weight[i][l]>-1.){
					Hu_Exp[i][l] += 1./iCount* Hu[j][l];	
					Hd_Exp[i][l] += 1./iCount * Hd[j][l];	
		...
		for(int l=0;l<100;l++){
			for(int j=0;j<N_W;j++){
				if(Weight[i][l]>-1.){
					Hu_Vol[i][l] += 1./iCount* pow((Hu[l][j]-Hu_Exp[i][l]),2);
					Hd_Vol[i][l] += 1./iCount* pow((Hd[l][j]-Hd_Exp[i][l]),2);
